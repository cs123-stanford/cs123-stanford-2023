...
#. In the Command Pallette, select the Remote-to-Host option, and type in ``pi@raspberrypi.local`` to prompt a remote connection. This will allow you to ssh into the Pupper's Raspberry Pi via the ethernet connection. 
...
Copy in the line ``export PYTHONPATH=/home/pi/CS123/lab_7_llms`` to the end of this file (you can just copy and paste in). Use Ctrl+O, Enter, Ctrl+X to write your changes. Source these changes in the terminal so that the Python path changes permanently. 
